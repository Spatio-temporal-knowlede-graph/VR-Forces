...
_RE_DISPLAY_NAME = re.compile(r'\(display-name\s+"([^"]*)"')
_RE_MUNITION = re.compile(r'\((\S+)\s+\(resource-type "munition-resource"\)')
...
class Golden:
    ...
    def weapons_of(self, dis: tuple[int, ...]) -> set[str]:
        """그 DIS 모델이 실제로 가진 무기 이름.

        `.oob` 레코드의 `(display-name "...")`이 VR-Forces가 태스크에서 쓰는
        무기 이름이다(실측: US Army M4 = "M4 rifle", Russian Soldier AK47 =
        "AK-47", 포병 = "Indirect-Fire-Gun"). 태스크가 지정한 무기가 여기에
        없으면 그 객체는 사격을 실행할 수 없다.
        """
        o = self.entity_by_dis(dis)
        if o is None:
            return set()
        return {m for m in _RE_DISPLAY_NAME.findall(o.raw) if m}

    def munitions_of(self, dis: tuple[int, ...]) -> set[str]:
        """그 모델이 가진 탄약 자원 이름. 간접사격 무기 문자열의 뒷부분이다
        (`Indirect-Fire-Gun:M107-155mm`의 `M107-155mm`)."""
        o = self.entity_by_dis(dis)
        if o is None:
            return set()
        return set(_RE_MUNITION.findall(o.raw))
