...
class WeaponRanges:
    def __init__(self) -> None:
        ...
        self._min_severity: dict[str, str] = {}
    ...
    def load(cls, path) -> "WeaponRanges":
        ...
        with open(Path(path), encoding="utf-8-sig", newline="") as f:
            for row in csv.DictReader(f):
                ...
                sev = (row.get("min_severity") or "").strip().upper()
                if sev:
                    wr._min_severity[key] = sev
    ...
    def min_severity(self, entity_class: str) -> str:
        """최소사거리 미달을 막을 것인가(BLOCK) 알리기만 할 것인가(REPORT).

        기본은 BLOCK이다. 지형이 좁아 교리상 최소사거리를 확보할 수 없다고
        사람이 판단한 모델만 표에서 REPORT로 내린다.
        """
        return self._min_severity.get(norm(entity_class), "BLOCK")
