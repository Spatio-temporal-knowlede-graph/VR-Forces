...
def unit_members(index: EventIndex, rules) -> dict[str, tuple[str, ...]]:
    ...
    객체처럼 다뤄지지만 부대가 아니다. 빼지 않으면 R6가 '사격진지 부대'의
    제압 비율을 재게 된다.
...
def r6_unit_suppressed(index: EventIndex, rules) -> RuleResult:
    ...
    한 명이고, 건수로 세면 소수가 반복 피격된 부대가 전멸한 것처럼 읽힌다.
    ...
    for rel in r1r2_hit_state(index, rules).relations:
        if rel.predicate == "suppresses":
            hit_by_member[rel.object].append(rel)

    rels = []
    for unit, roster in members.items():
        down = [oid for oid in roster if oid in hit_by_member]
        if not down or len(down) / len(roster) < ratio:
            continue
        why = tuple(p for oid in down
                    for rel in hit_by_member[oid] for p in rel.provenance)
        rels.append(Relation("R6", "unitSuppressed", unit, "제압", why))
